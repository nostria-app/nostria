...
describe('RepostButtonComponent', () => {
  ...
  let mockEventService: {
    loadReposts: jasmine.Spy;
    createNote: jasmine.Spy;
  };
  let mockRepostService: {
    repostNote: jasmine.Spy;
    deleteRepost: jasmine.Spy;
  };
  let mockLayoutService: {
    showLoginDialog: jasmine.Spy;
  };

  beforeEach(async () => {
    mockAccountState = {
      pubkey: signal('current-user-pubkey'),
      account: signal<{ pubkey: string; source: string; hasActivated: boolean } | null>({
        pubkey: 'current-user-pubkey',
        source: 'nsec',
        hasActivated: true,
      }),
    ...
    mockEventService = {
      loadReposts: jasmine.createSpy('loadReposts').and.returnValue(Promise.resolve([])),
      createNote: jasmine.createSpy('createNote').and.returnValue(Promise.resolve()),
    };

    mockRepostService = {
      repostNote: jasmine.createSpy('repostNote').and.returnValue(Promise.resolve(true)),
      deleteRepost: jasmine.createSpy('deleteRepost').and.returnValue(Promise.resolve(true)),
    };
  ...
  describe('createRepost', () => {
    ...
    it('should call repostNote and reload reposts for authenticated users', async () => {
      ...
      expect(mockEventService.loadReposts).toHaveBeenCalledWith(
        mockEvent.id, mockEvent.kind, 'current-user-pubkey', true
      );
    });
  });
