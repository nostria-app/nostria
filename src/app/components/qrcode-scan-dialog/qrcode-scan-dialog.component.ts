...
export class QrcodeScanDialogComponent implements AfterViewInit, OnDestroy {
      ...
      try {
        ...
        if (error instanceof Error) {
          if (error.name === 'NotAllowedError') {
            this.errorMessage.set('Camera access denied. Please allow camera permissions and try again.');
            ...
            this.errorMessage.set('Camera is already in use by another application. Close other apps using the camera and try a different camera below.');
          } else {
            this.errorMessage.set(`Camera error: ${error.message}`);
          }
        } else {
          this.errorMessage.set('Failed to access camera. Please ensure camera permissions are granted.');
        }
  ...
  private async initializeCameraWithRetry(video: HTMLVideoElement, maxRetries: number = 3): Promise<any> {
      ...
      try {
        ...
        if (error instanceof Error && error.name === 'NotReadableError') {
          ...
          if (this.availableCameras().length > 1 && attempt < maxRetries) {
            console.log('Multiple cameras available, will try with different camera on next attempt');
          }
        }
