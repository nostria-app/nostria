...
describe('RichTextEditorComponent', () => {
  ...
  const mockMediaService = {
    ...
    uploadFile: vi.fn().mockReturnValue(Promise.resolve({ status: 'success', item: { url: 'https://example.com/img.png' } })),
    mediaServers: vi.fn().mockReturnValue([]),
  };

  const mockLocalSettingsService = {
    removeTrackingParameters: vi.fn().mockReturnValue(false),
  };
  ...
  describe('media insertion', () => {
    it('should insert media links into the saved rich text cursor when focus moves outside the editor', () => {
      const emitted: string[] = [];
      component.contentChange.subscribe(value => emitted.push(value));
      ...
      (component as unknown as { richTextInsertionRange: Range }).richTextInsertionRange = editorRange;

      const externalElement = document.createElement('button');
      externalElement.textContent = 'Dialog button';
      document.body.appendChild(externalElement);
      const externalRange = document.createRange();
      externalRange.selectNodeContents(externalElement);
      const selection = window.getSelection();
      selection?.removeAllRanges();
      selection?.addRange(externalRange);

      (component as unknown as {
        insertFileLink(url: string, fileName: string, fileType: string): void;
      }).insertFileLink('https://example.com/document.pdf', 'document.pdf', 'application/pdf');

      expect(editor.innerHTML).toContain('href="https://example.com/document.pdf"');
      expect(emitted.at(-1)).toContain('[document.pdf](https://example.com/document.pdf)');
      expect(externalElement.innerHTML).toBe('Dialog button');
