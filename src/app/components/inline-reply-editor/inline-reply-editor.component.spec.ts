...
describe('InlineReplyEditorComponent', () => {
  ...
  const mockNostrService = {
    ...
    signAndPublish: jasmine.createSpy('signAndPublish'),
  };
  ...
  const mockMediaService = {
    ...
    getFileMimeType: jasmine.createSpy('getFileMimeType'),
  ...
  const mockPublishEventBus = {
    on: jasmine.createSpy('on').and.returnValue({ subscribe: () => ({ unsubscribe: jasmine.createSpy('unsubscribe') }) }),
  };
  ...
  const mockNoteEditorService = {
    getHashtagsFromContent: jasmine.createSpy('getHashtagsFromContent').and.returnValue([]),
    processContentForPublishing: jasmine.createSpy('processContentForPublishing').and.returnValue(''),
    detectMention: jasmine.createSpy('detectMention').and.returnValue({ isTypingMention: false }),
    sanitizeDisplayName: jasmine.createSpy('sanitizeDisplayName'),
    replaceMention: jasmine.createSpy('replaceMention'),
    loadProfileName: jasmine.createSpy('loadProfileName').and.returnValue(Promise.resolve(null)),
    buildTags: jasmine.createSpy('buildTags').and.returnValue([]),
  };
  ...
  const mockImagePlaceholder = {
    generatePlaceholders: jasmine.createSpy('generatePlaceholders'),
  };

  const mockUtilitiesService = {
    extractThumbnailFromVideo: jasmine.createSpy('extractThumbnailFromVideo'),
  };
