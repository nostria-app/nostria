...
describe('FeedService', () => {
  ...
  describe('shouldUseBackupFeeds', () => {
    it('should restore backup when current feeds only contain defaults', () => {
      const { service } = createServiceForFeedSyncTests();
      ...
      expect((service as any).shouldUseBackupFeeds(currentFeeds, backupFeeds)).toBe(true);
    });

    it('should not restore backup when current feeds already contain the same custom feed ids', () => {
      const { service } = createServiceForFeedSyncTests();
