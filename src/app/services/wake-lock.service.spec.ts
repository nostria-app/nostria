...
describe('WakeLockService', () => {
  ...
  beforeEach(async () => {
    ...
    mockWakeLock = {
      request: jasmine.createSpy('request').and.returnValue(Promise.resolve(mockWakeLockSentinel)),
    };
  ...
  it('should not acquire wake lock if already acquired', async () => {
    await service.enable();
    mockWakeLock.request.calls.reset();
    await service.enable();
  ...
  it('should handle unsupported browsers gracefully', async () => {
    ...
    await expectAsync(newService.enable()).toBeResolved();
    await expectAsync(newService.disable()).toBeResolved();
