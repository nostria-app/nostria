 ...
 */
async function performExtensionSigningFixed(state: {
    currentSigningDialogRef: MockDialogRef | null;
}, openDialog: () => MockDialogRef, signEventFn: () => Promise<{
    id: string;
}>): Promise<{
    id: string;
}> {
 ...
 */
async function performExtensionSigningBuggy(state: {
    currentSigningDialogRef: MockDialogRef | null;
}, openDialog: () => MockDialogRef, signEventFn: () => Promise<{
    id: string;
}>): Promise<{
...
describe('Extension signing queue dialog lifecycle', () => {
    it('FIXED: sequential signing requests should each close their dialog', async () => {
        ...
        const signPromiseA = new Promise<{
            id: string;
        }>(r => (resolveSignA = r));
        ...
        const signPromiseB = new Promise<{
            id: string;
        }>(r => (resolveSignB = r));

        // Start request A
        const resultAPromise = performExtensionSigningFixed(state, () => dialogA, () => signPromiseA);
        ...
        const resultBPromise = performExtensionSigningFixed(state, () => dialogB, () => signPromiseB);
    ...
    it('BUGGY (old code): previous dialog afterClosed nulls current dialog ref', async () => {
        ...
        // Start request A
        const resultAPromise = performExtensionSigningBuggy(state, () => dialogA, () => signPromiseA);
        ...
        const resultBPromise = performExtensionSigningBuggy(state, () => dialogB, () => signPromiseB);
    ...
    it('FIXED: user closing the active dialog should cancel signing', async () => {
        ...
        await expect(resultPromise).rejects.toThrowError('Signing cancelled by user');
        expect(state.currentSigningDialogRef).toBeNull();
    });
