...
import { Filter, finalizeEvent, generateSecretKey, getEventHash, getPublicKey, kinds, NostrEvent } from 'nostr-tools';
  ...
  async loadMoreChats(): Promise<void> {
    ...
    try {
      ...
      const sub = this.relay.subscribe([filterReceived, filterSent], async (event: NostrEvent) => {
        ...
        this.logger.debug(`EOSE received. Pending: ${pendingDecryptions}, Completed: ${completedDecryptions}`);
        eoseReceived = true;
        checkCompletion();
      });
