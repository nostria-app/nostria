...
interface AccountLocalState {
  ...
  podcastsListFilter?: string; // Filter for podcasts: 'curated', 'all', 'following', or follow set d-tag
...
export class AccountLocalStateService {
  ...
  /**
   * Get last viewed Clips For You event id for an account
   */
  getClipsLastForYouEventId(pubkey: string): string | undefined {
    const state = this.getAccountState(pubkey);
    return state.clipsLastForYouEventId;
  }

  /**
   * Set last viewed Clips For You event id for an account
   */
  ...
  /**
   * Get articles list filter for an account
   * Returns 'following' if not set (shows articles from following)
   */
  getArticlesListFilter(pubkey: string): string {
    const state = this.getAccountState(pubkey);
    return state.articlesListFilter || 'following';
  }

  /**
   * Set articles list filter for an account
   * @param filter - 'following', 'public', or a follow set d-tag
   */
  setArticlesListFilter(pubkey: string, filter: string): void {
    // Only store non-default values
  ...
  setPodcastsListFilter(pubkey: string, filter: string): void {
    const value = filter === 'curated' ? undefined : filter;
    this.updateAccountState(pubkey, { podcastsListFilter: value });
  }
  ...
  getCalendarListFilter(pubkey: string): string {
    const state = this.getAccountState(pubkey);
    return state.calendarListFilter || 'all';
  }
