...
function createComponent(overrides?: Partial<EventDetailsDialogData>): EventDetailsDialogComponent {
  ...
  (component as any).localSettings = {
    calendarType: jasmine.createSpy('calendarType').and.returnValue('gregorian'),
  };
  (component as any).chroniaCalendar = {
    fromDate: jasmine.createSpy('fromDate'),
...
describe('EventDetailsDialogComponent', () => {
  describe('isCurrentUserEvent', () => {
    it('should return true when currentUserPubkey matches event pubkey', () => {
      ...
      expect(component.isCurrentUserEvent).toBeTrue();
    });
    ...
    it('should return false when currentUserPubkey does not match event pubkey', () => {
      ...
      expect(component.isCurrentUserEvent).toBeFalse();
    });
  ...
  describe('isLocationUrl', () => {
    it('should return true for http URLs', () => {
      ...
      expect(component.isLocationUrl('http://example.com')).toBeTrue();
    });

    it('should return true for https URLs', () => {
      const component = createComponent();
      expect(component.isLocationUrl('https://example.com/venue')).toBeTrue();
    });

    it('should return false for plain text', () => {
      const component = createComponent();
      expect(component.isLocationUrl('Berlin, Germany')).toBeFalse();
    });

    it('should return false for non-http protocols', () => {
      const component = createComponent();
      expect(component.isLocationUrl('ftp://example.com')).toBeFalse();
    });
  });
  ...
  describe('formatDate', () => {
    ...
    it('should use chronia calendar when calendarType is chronia', () => {
      const component = createComponent();
      (component as any).localSettings.calendarType.and.returnValue('chronia');
      (component as any).chroniaCalendar.fromDate.and.returnValue({ year: 2025, month: 6, day: 15 });
      (component as any).chroniaCalendar.format.and.returnValue('Chronia Date');
    ...
    it('should use ethiopian calendar when calendarType is ethiopian', () => {
      const component = createComponent();
      (component as any).localSettings.calendarType.and.returnValue('ethiopian');
      (component as any).ethiopianCalendar.fromDate.and.returnValue({ year: 2017, month: 10, day: 8 });
      (component as any).ethiopianCalendar.format.and.returnValue('Ethiopian Date');
