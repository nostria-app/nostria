...
describe('ProfileMediaComponent', () => {
  ...
  const mockProfileState = {
    ...
    increaseMediaDisplayLimit: jasmine.createSpy('increaseMediaDisplayLimit'),
    loadMoreMedia: jasmine.createSpy('loadMoreMedia').and.returnValue(Promise.resolve([])),
  };
  ...
  const mockLayoutService = {
    ...
    refreshLeftPanelScroll: jasmine.createSpy('refreshLeftPanelScroll'),
    refreshRightPanelScroll: jasmine.createSpy('refreshRightPanelScroll'),
  };
  ...
  beforeEach(async () => {
    ...
    // Reset spies
    mockProfileState.increaseMediaDisplayLimit.calls.reset();
    mockProfileState.loadMoreMedia.calls.reset();
    mockProfileState.loadMoreMedia.and.returnValue(Promise.resolve([]));
    mockLayoutService.refreshLeftPanelScroll.calls.reset();
    mockLayoutService.refreshRightPanelScroll.calls.reset();
  ...
  describe('computed signals', () => {
    ...
    it('should expose hasMoreToDisplay from profile state', () => {
      expect(component.hasMoreToDisplay()).toBeFalse();
      mockHasMoreMediaToDisplay.set(true);
      expect(component.hasMoreToDisplay()).toBeTrue();
    });
  ...
  describe('loadMore', () => {
    ...
    it('should set error signal on failure', async () => {
      mockProfileState.loadMoreMedia.and.returnValue(Promise.reject(new Error('Network error')));
      await component.loadMore();
