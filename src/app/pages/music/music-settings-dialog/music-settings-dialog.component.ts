...
@Component({
  ...
  styleUrl: './music-settings-dialog.component.scss',
})
export class MusicSettingsDialogComponent implements OnInit {
  ...
  musicRelaySet = signal<MusicRelaySet>({ event: null, relays: [] });
  ...
  constructor() {
    // Effect to sync relays when musicRelaySet changes
    effect(() => {
      const set = this.musicRelaySet();
  ...
  private async loadMusicRelaySet(): Promise<void> {
    ...
    try {
      ...
      // Query for the user's music relay set (kind 30002 with d tag "music")
      const filter: Filter = {
        kinds: [RELAY_SET_KIND],
        authors: [pubkey],
        '#d': [MUSIC_RELAY_SET_D_TAG],
        limit: 1,
      };
  ...
  async saveRelaySet(): Promise<void> {
    ...
    try {
      ...
      const tags: string[][] = [
        ['d', MUSIC_RELAY_SET_D_TAG],
        ['title', 'Music Relays'],
        ['description', 'Relays for music content discovery'],
      ...
      await this.database.saveEvent({ ...signedEvent, dTag: MUSIC_RELAY_SET_D_TAG });

      this.snackBar.open('Music relay settings saved!', 'Dismiss', { duration: 3000 });
      this.hasExistingRelaySet.set(true);
      this.musicRelaySet.set({
        event: signedEvent,
        relays: this.relays(),
      ...
      this.logger.error('Error saving music relay set:', error);
  ...
  get suggestedRelays(): string[] {
    return DEFAULT_MUSIC_RELAYS.filter(r => !this.relays().includes(r));
  }
}
