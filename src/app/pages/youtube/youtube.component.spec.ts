...
function createComponent(): YouTubeComponent {
    ...
    (component as any).nostrService = {
        createEvent: vi.fn().mockImplementation((kind: number, content: string, tags: string[][]) => ({
            kind,
            content,
            tags,
            pubkey: 'test-pubkey',
...
describe('YouTubeComponent', () => {
    describe('loadYouTubeBookmarks', () => {
        it('should load channels from a kind 30078 event', async () => {
            ...
            (component as any).accountRelay.getMany.mockResolvedValue([event]);
            (component as any).corsProxy.fetchText.mockResolvedValue('<feed></feed>');
        ...
        it('should set empty channels when no events are found', async () => {
            ...
            (component as any).accountRelay.getMany.mockResolvedValue([]);
        ...
        it('should use the most recent event when multiple exist', async () => {
            ...
            (component as any).accountRelay.getMany.mockResolvedValue([olderEvent, newerEvent]);
            (component as any).corsProxy.fetchText.mockResolvedValue('<feed></feed>');
    ...
    describe('removeChannel', () => {
        ...
        it('should publish a kind 30078 event when removing a channel', async () => {
            ...
            expect((component as any).nostrService.createEvent).toHaveBeenCalledWith(30078, JSON.stringify([secondEntry]), [['d', 'youtube-channels']]);
        ...
        it('should show snackbar message on successful removal', async () => {
            ...
            expect((component as any).snackBar.open).toHaveBeenCalledWith('YouTube channel removed.', 'Close', { duration: 3000 });
        });
        ...
        it('should show error snackbar when removal fails', async () => {
            ...
            (component as any).nostrService.signEvent.mockRejectedValue(new Error('Sign failed'));

            await component.removeChannel('UC1XvxnHFtWruS9egyFasP1Q');

            expect((component as any).snackBar.open).toHaveBeenCalledWith('Failed to remove channel. Please try again.', 'Close', { duration: 3000 });
        });
    });
    ...
    describe('publishYouTubeEvent', () => {
        it('should create a kind 30078 event with d-tag youtube-channels', async () => {
            ...
            expect((component as any).nostrService.createEvent).toHaveBeenCalledWith(30078, JSON.stringify([sampleEntry]), [['d', 'youtube-channels']]);
        });
        ...
        it('should throw when signing fails', async () => {
            const component = createComponent();
            (component as any).nostrService.signEvent.mockResolvedValue(null);

            await expect((component as any).publishYouTubeEvent([sampleEntry])).rejects.toThrowError('Failed to sign event');
        });
    });
    ...
    describe('checkForOldBookmarks', () => {
        it('should count old bookmarks that are not already imported', async () => {
            ...
            (component as any).accountRelay.getMany.mockResolvedValue([oldEvent]);
    ...
    describe('migrateOldBookmarks', () => {
        it('should migrate old bookmarks to the new format', async () => {
            ...
            (component as any).accountRelay.getMany.mockImplementation((filter: any) => {
            ...
            expect((component as any).nostrService.createEvent).toHaveBeenCalledWith(30078, JSON.stringify([{
                    channelId: 'UCLA_DiR1FfKNvjuUpBHmylQ',
                    title: 'NASA',
                    description: 'NASA channel',
                    image: 'https://example.com/nasa.jpg',
            ...
            expect((component as any).snackBar.open).toHaveBeenCalledWith('Migrated 1 channel successfully!', 'Close', { duration: 3000 });
            expect(component.oldBookmarkCount()).toBe(0);
        });
        ...
        it('should skip channels already in the new format', async () => {
            ...
            expect((component as any).snackBar.open).toHaveBeenCalledWith('All channels already imported.', 'Close', { duration: 3000 });
        });
        ...
        it('should show message when no old bookmarks found', async () => {
            ...
            expect((component as any).snackBar.open).toHaveBeenCalledWith('No old bookmarks found.', 'Close', { duration: 3000 });
        });
        ...
        it('should handle multiple old bookmarks and deduplicate', async () => {
            ...
            expect((component as any).snackBar.open).toHaveBeenCalledWith('Migrated 2 channels successfully!', 'Close', { duration: 3000 });
        });
        ...
        it('should show error snackbar when migration fails', async () => {
            ...
            expect((component as any).snackBar.open).toHaveBeenCalledWith('Failed to migrate bookmarks. Please try again.', 'Close', { duration: 3000 });
            expect(component.migrating()).toBe(false);
        });
    });
});
